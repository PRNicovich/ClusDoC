function ResultTable=Fun_Map_DoC_GUIV2(CellData, NDatacolumns, ROICoordinates, DensityROI, Path_name, ColoThres)
...
Result = struct('Correlation_Coloc1_vs_Density1', [], 'Correlation_Coloc2_vs_Density2', [], ...
        ...
        for cell = 1:size(CellData, 1)
            ...
            for reg = 1:size(ROICoordinates{cell}, 1)
                ...
                DataRaw = CellData{cell}(whichPointsInROI, :);

                Data_DoC1 = DataRaw(DataRaw(:, NDatacolumns + 7) == 1, :);
%%      
...
%                 disp('line283');
                ...
                % Density Ch1 + Ch2
                Result.AvNorm_DensityAbove04 = mean(Data_CoC1.NormDensity(Data_DoC1.DoC >= ColoThres));
                Result.AvRela_DensityAbove04 = mean(Data_DoC1.Density(Data_DoC1.DoC >= ColoThres) / DensityROI{reg,cell}(1));
                Result.AvNorm_DensityBelow04 = mean(Data_CoC1.NormDensity(Data_DoC1.DoC < ColoThres));
                Result.AvRela_DensityBelow04 = mean(Data_DoC1.Density(Data_DoC1.DoC < ColoThres) / DensityROI{reg,cell}(1));
                
                Result.Correlation_ColocVsDensity = corr(Data_DoC1.Density, Data_DoC1.DoC, 'Type', 'Spearman');
