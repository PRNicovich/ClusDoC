...
% Data_Doc=table [X Y Lr Ch Density DoC D1_D2];
...
for i=1:max(class)
    ...
    %% Clusters with DoC >0.4 
    
    
    Point_In=[Data_DoCi.X(Data_DoCi.DoC>=0.4) Data_DoCi.Y(Data_DoCi.DoC>=0.4)];
    Nb_In=size(Point_In,1);
    
    if Nb_In>1
    Density20_In=Density20(Data_DoCi.DoC>=0.4);
    ...
    Density20_Out=Density20(Data_DoCi.DoC<0.4);
