function [ClusImage,  AreaC, Circularity, Nb, Contour, edges, Cutoff_point] = Smoothing_fun4cluster(ClusterOI, display1, display2)
...
% ClusterOI=Cluster{330};
    ...
    xcenter = (xmax + xmin)/2;
    ycenter = (ymax + ymin)/2;
    epsilon = 30;                   %%%%%%% HARDCODED PARAMETER
    xminbox = xcenter - (0.5*boxsize + epsilon);
    xmaxbox = xcenter + (0.5*boxsize + epsilon);
    yminbox = ycenter - (0.5*boxsize + epsilon);
    ymaxbox = ycenter + (0.5*boxsize + epsilon);
