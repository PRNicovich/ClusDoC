function ResultTable=Fun_Map_DofC_GUIV2(ROIData,Data_DofC, DensityROI)
...
%         end
mkdir('Statistic and Plots for Colocalization');
cd('Statistic and Plots for Colocalization')

mkdir('Raw data 2 Colors'); % Plot 1
mkdir('Removed Outliers 2 Colors'); % Plot 2
mkdir('Colocalisation and Density map'); % Plot 3
mkdir('Colocalistion with Threshold'); % Plot 4
mkdir('Colocalisation and Density map per Ch'); % Plot 5


Plot1=1; % Raw data 2 Colors
Plot2=1; % 2 colors Removed outliers Data (DataNoOutliers)
Plot3=1; % Colocalisation and Density map: 2 colors, 'No Outliers', colocalisation map + density map
Plot4=1; % Colocalistion with Threshold: 2 colors 'No Outliers' threshold with colocalisation>0.4(ColoThres)
         %
...
%                 DataNoOutliers(col,:)=[];
                ...
                x1=Data_DofC1.X(Data_DofC1.Ch==1); %  data from channel 1
                y1=Data_DofC1.Y(Data_DofC1.Ch==1); %  data from channel 2
                x2=Data_DofC1.X(Data_DofC1.Ch==2);
                y2=Data_DofC1.Y(Data_DofC1.Ch==2);
          ...
          %%    Map of Degree of Colocalisation and Density (+ Normalisation)
                ...
                % Channel Ch1
                D1=Data_DofC1.D1_D2(Data_DofC1.Ch==1);
                DofC1=Data_DofC1.DofC (Data_DofC1.Ch==1);
                D1_Norm=D1/max(D1);
                
                Result.Correlation_Coloc1_vs_Density1=corr(D1,DofC1,'Type','Spearman');
                
                % Channel Ch2                
                D2=Data_DofC1.D1_D2(Data_DofC1.Ch==2);
                DofC2=Data_DofC1.DofC (Data_DofC1.Ch==2);
                D2_Norm=D2/max(D2);
                
                Result.Correlation_Coloc2_vs_Density2=corr(D2,DofC2,'Type','Spearman');
...
%%                                                
                ...
                % Colocalisation and Density map per C   
                    ...
                    tt = getframe(gcf);
                    imwrite(tt.cdata, strcat('Colocalisation and Density map per Ch\'...
                        ,GenericName ,'DofC_Ch',num2str(1),'.tif'))
                    %saveas(gcf,strcat('Plot6\',GenericName ,'Plot6',strcat('_Ch',num2str(i))), 'tif');
                    close gcf
                    
                    figure
                    scatter( x2, y2,2,DofC2); 
                    axis equal
                    axis tight 
                    xlim(Xlimit);
                    ylim(Ylimit);
                    ...
                    tt = getframe(gcf);
                    imwrite(tt.cdata, strcat('Colocalisation and Density map per Ch\'...
                        ,GenericName ,'DofC_Ch',num2str(2),'.tif'))
                    %saveas(gcf,strcat('Plot6\',GenericName ,'Plot6',strcat('_Ch',num2str(i))), 'tif');
                    close gcf
                end
...
%                 Result.PercentLrAbove05_Ch2=length(find(Lr2Norm>0.5))/length(Lr2);
                ...
                % Above threshold
                D1_above_04=Data_DofC1.D1_D2(Data_DofC1.Ch==1 & Data_DofC1.DofC>=ColoThres);
                ...
                % Below Threshold
                D1_below_04=Data_DofC1.D1_D2(Data_DofC1.Ch==1 & Data_DofC1.DofC<ColoThres);
                ...
                % Above threshold
                D2_above_04=Data_DofC1.D1_D2(Data_DofC1.Ch==2 & Data_DofC1.DofC>=ColoThres);
                ...
                % Below Threshold
                D2_below_04=Data_DofC1.D1_D2(Data_DofC1.Ch==2 & Data_DofC1.DofC<ColoThres);
...
%    PercentLrAbove05_Ch2=arrayfun(@(x) x{:}.PercentLrAbove05_Ch2,ResultTable)
    ...
    Array1=[{'Percent DC>0.4 Ch1'},	{'Percent DC>0.4 Ch2'},	{'AvNormDensity>0.4'},	{'AvNormDensity<0.4'},...
    ...
    RegionName1=strcat('DC Results1'); % name of the sheet
    xlswrite('Region',Array1,RegionName1,'A1'); %'Regiion' = name of the filename xcel shee, Array1 = data to put in the spreadsheet, 'A1' where to start
    xlswrite('Region',Matrix_Result1,RegionName1,'A2');
