function [Lr_r_Result,r]=RipleyKmultiData_GUIFunV2 (AllData,Path_name)
        
        % create the output folder 'RipleyKGUI_Result
        Fun_OutputFolder_name=fullfile(Path_name,'RipleyKGUI_Result');
        if ~exist(Fun_OutputFolder_name,'dir')
            mkdir(Fun_OutputFolder_name);
        end
        
        sizeData=size(AllData);
        cd(Fun_OutputFolder_name);
        mkdir('RipleyK_Plots');
%%
i=0;
for p=1:sizeData(2)
...
annotation('textbox', [0.65,0.8,0.22,0.1],'String', str1,'FitBoxToText','on');
         ...
         set(gcf,'Color',[1 1 1])
         tt = getframe(gcf);
         imwrite(tt.cdata, strcat('RipleyK_Plots\','Ripley_Average.tif'))
         close gcf
